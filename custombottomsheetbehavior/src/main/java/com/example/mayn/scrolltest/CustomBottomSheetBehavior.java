...
public class CustomBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (mState == STATE_EXPANDED && savedTop == mParentHeight / 2) {
            //在半屏，防止子类（像ViewPager 设置的pageOffsetLimit!=size 导致要重新layout）重新layout时，被父view重新layout为吸顶
            ViewCompat.offsetTopAndBottom(child, mParentHeight / 2);
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        Log.d("tag", "behavior  onInterceptTouchEvent," + event.getAction());
        if (!child.isShown()) {
            mIgnoreEvents = true;
            return false;
        }
    ...
    public boolean onTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        Log.d("tag", "behavior  onTouchEvent," + event.getAction());
        if (!child.isShown()) {
            return false;
        }
        int action = event.getActionMasked();
        ...
        if (action == MotionEvent.ACTION_MOVE && !mIgnoreEvents) {
            if (Math.abs(mInitialY - event.getY()) > mViewDragHelper.getTouchSlop()) {
                Log.d("tag", "behavior onTouchEvent   Math.abs(mInitialY - event.getY()) > mViewDragHelper.getTouchSlop()");
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx,
        ...
        if (dy > 0) { // Upward
            Log.d("tag", "currentTop=" + currentTop + ",newTop=" + newTop + ",dy=" + dy);
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target) {
        ...
        if (mLastNestedScrollDy > 0) {
            ...
            Log.d("tag", "getYVelocity()===" + yVelocity);
            int centerInParent = mParentHeight / 2;
            if (yVelocity > 1500f || currentTop > centerInParent) {
                top = mMaxOffset;
                targetState = STATE_COLLAPSED;
            } else {
                top = centerInParent;
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (mViewRef.get() == null) return;
            int currentTop = mViewRef.get().getTop();
            int centerInParent = mParentHeight / 2;


            Log.d("tag", "currentTop=" + currentTop + ",mMinOffset=" + mMinOffset + ",mMaxOffset=" + mMaxOffset);

            @State int targetState;
            if (yvel < 0) { // Moving up

                Log.d("tag", "yvel < 0.f");
                if (currentTop >= centerInParent) {
                    top = centerInParent;
                    targetState = STATE_EXPANDED;
                } else {
                    top = mMinOffset;
               ...
               /* if (Math.abs(currentTop - mMinOffset) < Math.abs(currentTop - mMaxOffset)) {
                ...
                if (dy > 0) {
                    //向下
                    if (currentTop > mMinOffset && currentTop <= centerInParent) {
                        //顶部向下滑动
                        top = centerInParent;
                        targetState = STATE_EXPANDED;
                    } else {
                    ...
                    //向上
                    if (currentTop < mMaxOffset && currentTop >= centerInParent) {
                        //顶部向下滑动
                        top = centerInParent;
                        targetState = STATE_EXPANDED;
                    } else {
               ...
               /* Log.d("tag", "yvel == 0.f");
                if (Math.abs(currentTop - mMinOffset) < Math.abs(currentTop - mMaxOffset)) {
                    if (Math.abs(currentTop - mMinOffset) < Math.abs(currentTop - centerInParent)) {
                        Log.d("tag", "yvel == 0.f  if  if ");
                        top = mMinOffset;
                        targetState = STATE_EXPANDED;
                    } else {
                        Log.d("tag", "yvel == 0.f  if  else ");
                        top = centerInParent;
                        targetState = STATE_EXPANDED;
                    }

                } else {
                    Log.d("tag", "yvel == 0.f   else    ");
                    top = mMaxOffset;
                    targetState = STATE_COLLAPSED;
                }*/
            } else {
                //下滑
                Log.d("tag", "yvel > 0.f" + ",yvel====" + yvel);
